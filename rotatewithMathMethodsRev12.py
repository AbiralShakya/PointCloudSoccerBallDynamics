...
def find_clusters(mst, points, distance_threshold):
    ...
    def dfs(node, cluster):
        ...
        neighbors = np.argwhere(mst[node] > 0).flatten()
        for neighbor in neighbors:
            if neighbor not in visited and distances[node, neighbor] < distance_threshold:
                dfs(neighbor, cluster)
    ...
    for i in range(len(points)):
        if i not in visited:
            ...
            # Find indices of closest three points
            closest_indices = np.argsort(cluster_distances, axis=None)[:3]
            
            # Extract the indices of the closest three points
            closest_row_indices, closest_col_indices = np.unravel_index(closest_indices, cluster_distances.shape)
            
            # Keep only the unique points from the closest three
            cluster = np.unique(cluster[closest_row_indices])
...
def polar_to_cartesian(angle, distance):
    ...
    Returns:
    - Tuple representing Cartesian coordinates (x, y)
    """
...
def calculate_angle(point1, point2, point3):
    ...
    Args:
    - point1, point2, point3: tuples representing points in polar coordinates (angle, distance)
...
'''
def is_triangular_pattern(points, angle_threshold=100):
    if len(points) != 3:
        return False
...
def detect_black_points(roi, color_roi):
    ...
    _, mask = cv2.threshold(blurred, 220
                            , 255, cv2.THRESH_BINARY) #200 prev
    cv2.imshow('mask', mask)
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    #print("COUNTORUS", contours)
    points = []
    for contour in contours:
        ...
        if M["m00"] != 0:
            ...
            #points.append((cx,cy))
            #points.append((cx, cy))
            if is_surrounded_by_white(color_roi, cx, cy, white_threshold=50, radius=1, white_ratio_threshold=0.01):
                points.append((cx, cy))
...
def detect_black_points(roi, color_roi):
    inverted = cv2.bitwise_not(roi)
    blurred = cv2.GaussianBlur(inverted, (5, 5), 0)
    _, mask = cv2.threshold(blurred, 150, 255, cv2.THRESH_BINARY) #200 prev
    cv2.imshow('mask', mask)
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for contour in contours:
        ...
        if 3 < len(approx) < 7:
            ...
            if M["m00"] != 0:
                ...
                if is_surrounded_by_white(color_roi, cx, cy, white_threshold=50, radius=10, white_ratio_threshold=0.1):
                    points.append((cx, cy))
    return np.array(points)
'''
'''
def detect_black_points(roi, color_roi):
    inverted = cv2.bitwise_not(roi)
    blurred = cv2.GaussianBlur(inverted, (5, 5), 0)
    _, mask = cv2.threshold(blurred, 200, 255, cv2.THRESH_BINARY)  # 200 previous
    cv2.imshow('mask', mask)
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for contour in contours:
        ...
        if 1 < len(approx) <= 8:
            ...
            if distance < 100:  # Threshold for considering contour as closed   
                M = cv2.moments(contour)
                if M["m00"] != 0:
                    cx = int(M["m10"] / M["m00"])
                    cy = int(M["m01"] / M["m00"])
                    points.append((cx,cy))
                    #if is_surrounded_by_white(color_roi, cx, cy, white_threshold=50, radius=10, white_ratio_threshold=0.01):
                    #    points.append((cx, cy))
    return np.array(points)'''
...
def pixel_to_real_world(u, v, depth, intrinsics):
    ...
    X = (u - cx) * depth / fx
    Y = (v - cy) * depth / fy
    Z = depth  #TODO: revamp real_world conversions
...
def start_tracking():
    ...
    time_interval = 1 / 60
    prev_velocity = np.zeros((0, 3)) #irrelevant now
    mass = 0.5
...
def plot_3d_motion():
    ...
    ax.set_ylabel('Y axis')
    ax.set_zlabel('Z axis')
    ax.set_title('3D Motion vs Time')
...
'''def plot_rotation_matrix():
    ...
    data = np.array(plot_rotation_matrix_data)
    if data.size == 0:
        print("No rotation matrix data")
        return

    # Check and print the shape of the data for debugging
    print("Rotation matrix data shape:", data.shape)

    for i in range(data.shape[0]):
        ax.quiver(0, 0, 0, data[i, 0, 0], data[i, 1, 0], data[i, 2, 0], color='r')
        ax.quiver(0, 0, 0, data[i, 0, 1], data[i, 1, 1], data[i, 2, 1], color='g')
        ax.quiver(0, 0, 0, data[i, 0, 2], data[i, 1, 2], data[i, 2, 2], color='b')
...
def plot_energy():
    ...
    if isinstance(plot_translational_energy_data, list):
        ...
        data_to_plot = np.concatenate(plot_translational_energy_data)
    else:
        data_to_plot = plot_translational_energy_data
    ...
    plt.plot(data_to_plot)
    plt.xlabel('Time')
    plt.ylabel('Translational Energy')
    plt.title('Transalational Energy Over Time')
    plt.show()'''
